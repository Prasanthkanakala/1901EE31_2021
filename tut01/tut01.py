...
def meraki_helper(n):
    ...
    if(n<10):
	#case for numbers less than 10
        print("Yes-{} is a meraki number".format(n))
        return 1
    a=str(n)  #converting int value to a string
    i=0
    while(i<len(a)-1):
        if(abs(int(a[i])-int(a[i+1]))==1):
            #condition for checking whether the number is meraki...
	    i+=1
        else:
...
count1=0    #counter variable to keep the record of numbers
count2=0
for i in input:
    if(meraki_helper(i)):
        count1+=1
